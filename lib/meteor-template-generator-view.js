...
export default class MeteorTemplateGeneratorView {
  ...
  generateFileBasePath(dir,templateName,filetype) {
    return path.join(dir,templateName,templateName+"."+filetype);
  }
  ...
  generatePackage(name) {
    ...
    const jsPath = this.generateFileBasePath(path, name, "js");
    const htmlPath = this.generateFileBasePath(path, name, "html");

    const jsBuffer = new TextBuffer({text: this.getFileContentJs()});
    const htmlBuffer = new TextBuffer({text: this.getFileContentHTML()});

    jsBuffer.setPath(jsPath);
    htmlBuffer.setPath(htmlPath);

    atom.project.addBuffer(jsBuffer);
    atom.project.addBuffer(htmlBuffer);
